...
async function showMainScene(ctx) {
    ctx.deleteMessage(dataservice.getActiveMessageId(ctx.update.message.chat.id)).catch((e) => {
    ...
    ctx.reply(
        ...
        Markup.inlineKeyboard([
            ...
            Markup.callbackButton(showChatCounter(ctx.update.message.chat.id), 'show'),
            Markup.urlButton(`تعداد کل صلوات ها  ${toPersianDigits(globalCounter.toString())}`, 'https://salav.at')
        ], {columns: 1}).extra()
    ).then((ctx) => {
        console.log('active message id is :', ctx.chat)
...
function showChatCounter(id) {
    log(chalk.red('showing counter for the chat:'), id);
    switch (dataservice.getChatType(id)) {
        case 'private':
            return `شما تا الان ${toPersianDigits(dataservice.getCounter(id).toString())} صلوات فرستاده‌اید`
        case 'group':
            return `در این گروه تا الان ${toPersianDigits(dataservice.getCounter(id).toString())} صلوات فرستاده شده`
        case 'channel':
            return `در این کانال تا الان ${toPersianDigits(dataservice.getCounter(id).toString())} صلوات فرستاده شده`
    }
}
...
function postData(ctx, cbx, int) {
    ...
    fetch('https://api.salav.at/v1/update-data', {
        ...
        .then(res => res.json())
        .then(json => {
            dataservice.setCounter(cbx.update.callback_query.message.chat.id, dataservice.getCounter(cbx.update.callback_query.message.chat.id) + int)
            log(chalk.red('cbx:',cbx.update))
            cbx.editMessageText(
                `همین الان برای سلامتی و فرج امام‌زمان علیه‌السلام صلوات بفرست` + '\n' + ` توی سایت salav.at ثبت میشه`,
                Markup.inlineKeyboard([
                    Markup.callbackButton('یه ‌دونه فرستادم', 'one'),
                    Markup.callbackButton('ده ‌تا فرستادم', 'ten'),
                    Markup.callbackButton(showChatCounter(cbx.update.callback_query.message.chat.id), 'show'),
...
/start@Salav_at_bot
رو بزنید`
bot.command(['help', 'help@Salav_at_bot'], (ctx) => {
...
// /start command starts the bot for the user, has multiple triggers for private chats, groups and channels.
bot.command(['start', 'start@Salav_at_bot'], (ctx) => {
    ...
    if (dataservice.getUserList().includes(ctx.update.message.chat.id.toString())) {
        ...
        // if not available registering a new user
